...
class ValidationItemControllerV1(
  ...
  @PostMapping("/add")
  fun addItem(@ModelAttribute itemRequest: ItemRequest, redirectAttributes: RedirectAttributes, model: Model): String {
    //검증 오류 결과를 보관
    val errors: MutableMap<String, String> = HashMap()

    //검증 로직
    if (!StringUtils.hasText(itemRequest.name)) {
      errors["name"] = "상품 이름은 필수입니다. 현재 값 = ${itemRequest.name}"
    }
    if (itemRequest.price == null || itemRequest.price!! < 1000 || itemRequest.price!! > 1000000) {
      errors["price"] = "가격은 1,000 ~ 1,000,000 까지 허용합니다. 현재 값 = ${itemRequest.price}"
    }
    if (itemRequest.quantity == null || itemRequest.quantity!! >= 9999 || itemRequest.quantity!! < 10) {
      errors["quantity"] = "수량은 최대 9,999 까지 허용합니다. 현재 값 = ${itemRequest.quantity}"
    }
