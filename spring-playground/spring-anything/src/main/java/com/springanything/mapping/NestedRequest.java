...
1. @Setter 없애고 각각의 필드를 Outer 객체의 생성자로 받는다
- Outer 객체 @NoArgsConstructor(access = AccessLevel.PROTECTED), @AllArgsConstructor, @Getter
- Inner 객체 @NoArgsConstructor(access = AccessLevel.PROTECTED), @AllArgsConstructor(access = AccessLevel.PRIVATE), @Getter
이때 Controller에서 query-string ModelAttribute로 매핑한다면 아래와 같은 형태로 된다
/test/reiterative?name=panda&nickname=bear
즉 중첩 객체지만 innerRequest.name=xxx, innerRequest.nickname=xxx 형태로 매핑하지 않고 name=xxx, nickname=xxx 형태로 매핑한다
